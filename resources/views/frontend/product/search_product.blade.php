...
@if ($products->isEmpty())
    <h6 class="text-center text-danger">No Product Found</h6>
@else
                    ...
                    @foreach ($products as $item)
                        <a href="{{ url('product/details/' . $item->id . '/' . $item->product_slug) }}">
                            <div class="list border-bottom">
                                <img src="{{ asset('upload/product_images/thambnail/' . $item->product_thambnail) }}"
                                    alt="{{ $item->product_name }}" width="40px" height="40px">
